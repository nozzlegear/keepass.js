...
module Keepass {
     ...
     */
	export class MasterKeyUtil {
        private keyFileParser = new KeyFileParser();
        
		public inferMasterKey(h, masterPassword, keyFile?) {
            if (keyFile) {
                return this.keyFileParser.getKeyFromFile(keyFile).then((key) => {
                    return this.infer(h, masterPassword, key);
                });               
            }
            else {
